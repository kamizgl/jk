/**
 * 
 */
package cn.itcast.jk.action.cargo;

import java.util.Date;
import java.util.List;

import javax.servlet.http.HttpServletResponse;

import org.apache.struts2.ServletActionContext;

import cn.itcast.jk.action.BaseAction;
import cn.itcast.jk.domain.Contract;
import cn.itcast.jk.domain.Export;
import cn.itcast.jk.domain.User;
import cn.itcast.jk.pagination.Page;

import cn.itcast.jk.service.ContractService;
import cn.itcast.jk.service.ExportService;

import com.opensymphony.xwork2.ModelDriven;

/**
 * @description:
 * @author 传智.宋江
 * @date 2015年9月10日
 * @version 1.0
 * 
 * 思考问题：为什么继承BaseAction
 * 1.做到当前的Action与Struts2   Action的API解藕合
 * 2.可以在BaseAction中提供一些通用操作，这样子类就可以直接调用父类中的相关方法
 */
public class ExportAction extends BaseAction implements ModelDriven<Export> {
    private Export model = new Export();
	public Export getModel() {
		return model;
	}
	//分页
	private Page page = new Page();
	public Page getPage() {
		return page;
	}
	public void setPage(Page page) {
		this.page = page;
	}
	
	//引入业务逻辑
	private ExportService exportService ;
	public void setExportService(ExportService exportService) {
		this.exportService = exportService;
	}
	private ContractService contractService;
	public void setContractService(ContractService contractService) {
		this.contractService = contractService;
	}
	
	//分页查询部门列表    用户等级目前只考虑了员工4，部门经理3,其它没有考虑（   将来可以用1代表总经理   2代表副总）
	public String list() throws Exception {
		User user = super.getCurrentUser();//从session中获取登录的用户
		
		String hql = "from Export c where 1=1 ";
		
		
		page =  exportService.findPage(hql, page, null);
		
	    page.setUrl(ServletActionContext.getRequest().getContextPath()+"/cargo/exportAction_list");//设置上一页，下一页的链接
	    
		return "list";
	}
	
	//合同管理
	public String contractList() throws Exception {
		//1.要查询状态为已上报的购销合同
		List<Contract> list = contractService.find("from Contract c where c.state=1", Contract.class, null);
		super.put("dataList", list);
		return "contractList";
	}
	
	//查看详情
	public String toview() throws Exception {
		//接收参数的工作是ModelDriven拦截器干的   
		Export dept = exportService.get(model.getId());
		//将查询的详情加入到值栈的栈顶
		super.push(dept);
		return "toview";
	}
	
	//进入新增页面
	public String tocreate() throws Exception {
		
		
		return "tocreate";
	}
	
	//添加
	public String insert() throws Exception {
		//当实现报运时，就会产生一个出口报运单 ，当出口报运单生成时，同时还要实现打断设计中（打断字段的维护）
		//同时还要实现数据搬家（购销合同下的货物及附件要进行搬家，搬家到报运单下的货物及附件表中来）
		//同时还要将购销合同的状态改为已报运（state=2）
		exportService.save(model.getId().split(", "),model);   //model.getId().split(", ")购销合同的id
		return "alist";
	}
	
	//删除操作
	public String delete() throws Exception {
		//1.接收要删除的多个id的值----struts2框架可以接收
		//2.判断是删除一条还是多条
		String []ids = model.getId().split(", ");
		if(ids!=null && ids.length>1){
			//说明要删除多个部门
			exportService.delete(ids);
		}else{
			exportService.deleteById(model.getId());
		}
		return "alist";
	}
	
	//进入修改页面
	public String toupdate() throws Exception {
		//根据修改的部门id,查询出一个部门对象
		Export dept = exportService.get(model.getId());
		//2.将部门对象放入值栈的栈顶
		super.push(dept);
		
		//3.加载所有的部门
		List<Export> list = exportService.find("from Export o where o.state=1", Export.class, null);
		//4将部门列表放入值栈中
		super.put("deptList", list);
		
		
		return "toupdate";
	}
	
	//修改操作
	public String update() throws Exception {
		
		exportService.saveOrUpdate(model);
		return "alist";
	}
	
	
	//提交
	public String submit() throws Exception {
		changeState(1,0);
		return list();   //不走拦截器
	}
	
	//取消   
	public String cancel() throws Exception {
		changeState(0,1);
		return list();   //不走拦截器
	}
	/**
	 * 用于修改状态的方法
	 */
	private void changeState(int newState,int oldState) {
		String ids[] =  model.getId().split(", ");
		if(ids!=null && ids.length>1){
			for (String id :ids) {
				Export contract = exportService.get(id);
				if(contract.getState()==oldState){  //判断购销合同的状态，如果为已提交状态就可以取消，为其它状态就不可以再取消了
				    contract.setState(newState);
				    exportService.saveOrUpdate(contract);
				}
			}
		}else{
			//一条记录
			Export contract = exportService.get(model.getId());
			if(contract.getState()==oldState){ 
			     contract.setState(newState);
			     exportService.saveOrUpdate(contract);
			}
		}
	}
	
}
